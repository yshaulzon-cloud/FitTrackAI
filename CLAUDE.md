...
**Daily streak** (`areto_daily_streak` in `localStorage`) is computed client-side in `getDailyStreak()` at app open — counts consecutive calendar days the app was opened. The topbar flame icon (`FlameIcon`) renders a Canvas particle simulation (Perlin fbm noise, additive blending) sized to 5 tiers based on streak count.
